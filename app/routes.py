...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not next_page or urlsplit(next_page).netloc != '': # checks for th value of the `next` query string
            next_page = url_for('index') #+ parameter and ensures it is not an absolue url (security reasons)
...
@login_required
def explore():
    """renders the ecplore page with different post"""
    page = request.args.get('page', 1, type=int)
